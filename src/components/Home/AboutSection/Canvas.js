    ...
    document.addEventListener(
      ...
      ({ clientX, clientY }) => {
        if (window.innerWidth > 1024)
          addPoint(
            clientX - canvas.offsetLeft - 200,
            clientY - canvas.offsetTop
          );
        else addPoint(clientX - canvas.offsetLeft, clientY - canvas.offsetTop);
      },
      false
    );
    ...
    const animatePoints = () => {
      ...
      for (let i = 0; i < points.length; ++i) {
        ...
        if (point.lifetime > duration) {
          ...
          const spreadRate = 21 * (1 - lifePercent);

          ctx.lineJoin = 'round';
          ctx.lineWidth = spreadRate;

          // As time increases decrease r and b, increase g to go from purple to green.
          const red = Math.floor(244 - 244 * lifePercent);
          const green = 0;
          const blue = Math.floor(244 - 93 * lifePercent);
  ...
  render = () => {
    ...
    return (
      <canvas
        ...
        width={cWidth}
        height={cHeight}
        className="w-screen cursor-none"
      ></canvas>
    );
  };
}
