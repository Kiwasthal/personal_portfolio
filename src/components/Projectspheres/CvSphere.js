...
const CvSphere = () => {
  ...
  before:absolute before:top-0 before:left-0 before:w-full before:h-full before:bg-orange-500 before:clipCon before:transition-all before:duration-500
  group-hover:before:clipConHov "
          ref={ref}
          variants={sphereVariants}
          animate={controls}
        >
          <span className="relative  w-44 text-center text-3xl text-white font-logo rotate-350 transition-all sclae-1 duration-500 delay-0 group-hover:transition-all group-hover:scale-0 group-hover:delay-0 ">
            CV Application
          </span>
        </motion.div>
        <div className="w-1/2 h-full relative left-[30%] pt-9 pr-3 pb-3 pl-12 opacity-0 transition-all  invisible group-hover:left-0 group-hover:opacity-100 group-hover:visible group-hover:transition-all group-hover:delay-500 ">
          <h2 className="text-black font-bold text-2xl leading-4">
            CV Builder
          </h2>
          <span className="text-white font-bold leading-3 text-sm">React</span>
          <p className="text-black font-bold mb-4 mt-1 leading-5">
            An interactive cv-building application created as react practice on
            hooks. The user can add and update all fields with his preferred
            information.
          </p>
          <a
            ...
            className=" text-lg text-white font-bold relative bg-black px-4 py-2 inline-block rounded-md"
          >
            Client
          </a>
        </div>
        <img
          src={cvGif}
          alt="Fantasy-app gif, browsing website's features"
